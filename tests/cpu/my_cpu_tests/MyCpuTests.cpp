...
NES::NesSystem emulate(const std::vector<std::uint8_t>& program) {
    NES::NesSystem nes;
    nes.loadRom(program);
...
TEST_CASE("Check if 4 is even", "[cpu_basic]") {
    ...
    std::vector<std::uint8_t> program = {0xA9, 0x01, 0x8D, 0x00, 0x00, 0xA9, 0x04, 0x2D, 0x00, 0x00, 0xF0, 0x08, 0xA9,
...
TEST_CASE("Check if 5 is even", "[cpu_basic]") {
    ...
    std::vector<std::uint8_t> program = {0xA9, 0x01, 0x8D, 0x00, 0x00, 0xA9, 0x05, 0x2D, 0x00, 0x00, 0xF0, 0x08, 0xA9,
                                         0x01, 0x8D, 0x01, 0x00, 0x4C, 0x19, 0x80, 0xA9, 0x02, 0x8D, 0x01, 0x00};
